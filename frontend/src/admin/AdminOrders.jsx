...
const AdminOrders = ()=>{
    ...
    const updateStatus =async (id,status)=>{
        ...
        if (res.ok) {
            setOrders(setOrders.map(order=>order._id===id ? {...order,status} : order));            
        }
    };
    return (
              ...
              <tr key={order._id} style={rowStyle}>
                ...
                <td style={tdStyle}>
                  <select 
                    ...
                    style={{ background: '#24311f', color: '#fff', padding: '6px', border: '1px solid #22311d', borderRadius: '4px', outline: 'none' }}
                  >
...
const containerStyle = { maxWidth: '1200px', margin: '40px auto', padding: '30px', background: '#24311f', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)', color: '#fafafa' };
const tableStyle = { width: '100%', borderCollapse: 'collapse' };
const rowStyle = { borderBottom: '1px solid rgba(186, 194, 40, 0.1)' };
